...
class Build(build):
    def run(self):
        if path.isdir("queries"):
            dest = path.join(self.build_lib, "tree_sitter_roxygen2", "queries")
...
setup(
    ...
    package_data={
        "tree_sitter_roxygen2": ["*.pyi", "py.typed"],
        "tree_sitter_roxygen2.queries": ["*.scm"],
    },
    ext_package="tree_sitter_roxygen2",
    ext_modules=[
        Extension(
            name="_binding",
            sources=[
                "bindings/python/tree_sitter_roxygen2/binding.c",
                "src/parser.c",
            ],
            define_macros=[
